...
#ifdef SAMSUNG_PRODUCT_FEATURES
#include "SecProductFeature_WLAN.h"
#endif
...
void lease_update_file(time_t now)
{
  ...
  if (file_dirty != 0 && daemon->lease_stream)
	...
	err = errno;
      ...
      if (!err)
	file_dirty = 0;
#ifdef SEC_PRODUCT_FEATURE_WLAN_SEC_MOBILEAP
		my_syslog(MS_DHCP | LOG_ERR, _("ndc softap sta_join"));
